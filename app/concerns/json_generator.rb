  ...
  module TimelineModule
    ...
    def generateTimeline(rounds)
      ...
      rounds.each do |round|
        data << {:id => round.event_id, :data => "#{trunc(round.event_description, 125)} #{round.type}: #{trunc(round.related_obj.to_s,125)} - #{round.created_at.to_time.strftime(format) } ago", :attr => { :rel => round.type, :href => round.related_link }}
      end
      return data.to_json
    end
  end
